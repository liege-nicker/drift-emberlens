...
const sdk = new CoinbaseWalletSDK({
  appName: "Drift Emberlens (Built for Base)",
...
async function readErc20(token: string, holder: string) {
  ...
  print([
    ...
    `Token on Basescan: ${active.explorer}/address/${tokenAddr}`,
    `Holder on Basescan: ${active.explorer}/address/${holderAddr}`,
  ]);
}
...
function mount() {
  ...
  h1.textContent = "Drift Emberlens";
